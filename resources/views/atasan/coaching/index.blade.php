...
<div class="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8">
  ...
  <tr class="align-top"><td class="border border-slate-700 px-3 py-2 text-center">{{ $index + 1 }}</td><td class="border border-slate-700 px-3 py-2"><div class="font-semibold">{{ $plan->kompetensi?->kode_kompetensi ?? '-' }}</div><div>{{ $plan->kompetensi?->nama_kompetensi ?? '-' }}</div></td><td class="border border-slate-700 px-3 py-2">{{ $plan->pembelajaran_10_persen ?? 'Belum dibuat' }}</td><td class="border border-slate-700 px-3 py-2">@php($bukti10 = $plan->coachingBukti->where('jenis', 10)->first()) @if($bukti10)<span class="rounded bg-green-100 px-2 py-1 text-xs font-semibold text-green-700">{{ $bukti10->original_name ?? 'File uploaded' }}</span> <a class="ml-2 text-xs font-semibold text-blue-600 hover:text-blue-800" href="{{ route('atasan.coaching.download', ['idp' => $row->id_daftar_idp, 'type' => '10', 'idRencana' => $plan->id_rencana]) }}">Download</a>@endif <p class="mt-2 whitespace-pre-line text-xs text-slate-600">{{ $plan->deskripsi_realisasi_10 ?: 'Belum ada deskripsi' }}</p></td><td class="border border-slate-700 px-3 py-2">{{ $plan->social_learning_20_persen ?? 'Belum dibuat' }}</td><td class="border border-slate-700 px-3 py-2">@php($bukti20 = $plan->coachingBukti->where('jenis', 20)->first()) @if($bukti20)<span class="rounded bg-green-100 px-2 py-1 text-xs font-semibold text-green-700">{{ $bukti20->original_name ?? 'File uploaded' }}</span> <a class="ml-2 text-xs font-semibold text-blue-600 hover:text-blue-800" href="{{ route('atasan.coaching.download', ['idp' => $row->id_daftar_idp, 'type' => '20', 'idRencana' => $plan->id_rencana]) }}">Download</a>@endif <p class="mt-2 whitespace-pre-line text-xs text-slate-600">{{ $plan->deskripsi_realisasi_20 ?: 'Belum ada deskripsi' }}</p></td><td class="border border-slate-700 px-3 py-2">{{ $plan->action_learning_70_persen ?? 'Belum dibuat' }}</td><td class="border border-slate-700 px-3 py-2">@php($bukti70 = $plan->coachingBukti->where('jenis', 70)->first()) @if($bukti70)<span class="rounded bg-green-100 px-2 py-1 text-xs font-semibold text-green-700">{{ $bukti70->original_name ?? 'File uploaded' }}</span> <a class="ml-2 text-xs font-semibold text-blue-600 hover:text-blue-800" href="{{ route('atasan.coaching.download', ['idp' => $row->id_daftar_idp, 'type' => '70', 'idRencana' => $plan->id_rencana]) }}">Download</a>@endif <p class="mt-2 whitespace-pre-line text-xs text-slate-600">{{ $plan->deskripsi_realisasi_70 ?: 'Belum ada deskripsi' }}</p></td></tr>
  @empty
