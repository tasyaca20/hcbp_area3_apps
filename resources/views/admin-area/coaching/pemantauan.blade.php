...
<div class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
  ...
  @php($statusCounts = collect($statuses)->mapWithKeys(fn ($status) => [$status => $summaryRows->filter(fn ($row) => $row->monitoringCoaching?->status === $status || (!$row->monitoring && $status === $statuses[0]))->count()]))
    ...
    <table data-table-pagination="false" class="min-w-[1850px] w-full text-left text-xs">
      ...
      <tbody class="divide-y divide-slate-100">
        @forelse($rows as $index => $row)
          ...
          @php($status = $row->monitoringCoaching?->status ?? 'Belum Mengisi Sesi Coaching')
          @if($plans->isNotEmpty())
            ...
            <tr class="hover:bg-slate-50">
              ...
              @endif
              <td class="px-4 py-4"><div class="font-medium">{{ $plan->kompetensi?->kode_kompetensi ?? '-' }}</div><div class="text-slate-500">{{ $plan->kompetensi?->nama_kompetensi ?? '-' }}</div></td><td class="px-4 py-4">{{ $plan->pembelajaran_10_persen ?? '-' }}</td><td class="px-4 py-4">@php($bukti10 = $plan->coachingBukti->firstWhere('jenis', 10)) @if($bukti10)<a class="font-semibold text-blue-600 hover:underline" href="{{ route('admin-area.coaching.download', ['idp' => $row, 'type' => 10, 'idRencana' => $plan]) }}">PDF</a>@else-@endif</td><td class="px-4 py-4">{{ $plan->social_learning_20_persen ?? '-' }}</td><td class="px-4 py-4">@php($bukti20 = $plan->coachingBukti->firstWhere('jenis', 20)) @if($bukti20)<a class="font-semibold text-blue-600 hover:underline" href="{{ route('admin-area.coaching.download', ['idp' => $row, 'type' => 20, 'idRencana' => $plan]) }}">PDF</a>@else-@endif</td><td class="px-4 py-4">{{ $plan->action_learning_70_persen ?? '-' }}</td><td class="px-4 py-4">@php($bukti70 = $plan->coachingBukti->firstWhere('jenis', 70)) @if($bukti70)<a class="font-semibold text-blue-600 hover:underline" href="{{ route('admin-area.coaching.download', ['idp' => $row, 'type' => 70, 'idRencana' => $plan]) }}">PDF</a>@else-@endif</td>
            </tr>
            @endforeach
          @else
