        ...
        @forelse($rows as $index => $row)
          ...
          @if($plans->isNotEmpty())
            ...
            <tr class="hover:bg-slate-50">
              ...
              @endif
              <td class="px-4 py-4"><div class="font-medium">{{ $plan->kompetensi?->kode_kompetensi ?? '-' }}</div><div class="text-slate-500">{{ $plan->kompetensi?->nama_kompetensi ?? '-' }}</div></td><td class="px-4 py-4">{{ $plan->pembelajaran_10_persen ?? '-' }}</td><td class="px-4 py-4">{{ $row->monitoring?->pembelajaran_10_persen ?? '-' }}</td><td class="px-4 py-4">{{ $plan->social_learning_20_persen ?? '-' }}</td><td class="px-4 py-4">{{ $row->monitoring?->social_learning_20_persen ?? '-' }}</td><td class="px-4 py-4">{{ $plan->action_learning_70_persen ?? '-' }}</td><td class="px-4 py-4">{{ $row->monitoring?->experimental_learning_70_persen ?? '-' }}</td>
            </tr>
            @endforeach
          @else
